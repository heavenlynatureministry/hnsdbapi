...
def _get_current_academic_year() -> str:
    now = datetime.utcnow(); year = now.year; month = now.month
...
def _get_student_id_number(student: dict) -> str:
    return str(student.get("student_id") or student.get("student_id_number") or student.get("id_number") or student.get("admission_number") or str(student.get("_id", "")))
...
def _get_next_class_name(class_name: str) -> str:
    ...
    for key, value in progression.items():
        if key.upper() in upper:
            return upper.replace(key.upper(), value)
...
async def _calculate_position(db, student_oid, class_id, overall_percentage, term, academic_year) -> tuple:
    try:
        classmates = await db.students.find({"current_class_id": class_id, "status": "active"}).to_list(length=None)
        total_students = len(classmates)
        if total_students <= 1: return "1", str(total_students)
        class_percentages = []
        for classmate in classmates:
            c_oid = classmate["_id"]
            c_results = await db.exam_results.find({"student_id": c_oid, "term": term, "academic_year": academic_year}).to_list(length=None)
            if not c_results: c_results = await db.exam_results.find({"student_id": c_oid}).to_list(length=None)
            ...
            if c_total_max > 0: class_percentages.append(round((c_total_score / c_total_max * 100), 1))
            else: class_percentages.append(0)
...
async def _get_student_attendance(db, student_oid, term, academic_year) -> dict:
    records = await db.attendance.find({"student_id": student_oid, "term": term, "academic_year": academic_year}).to_list(length=None)
    if not records: records = await db.attendance.find({"student_id": student_oid, "academic_year": academic_year}).to_list(length=None)
    if not records:
        year_start = academic_year.split('/')[0] if '/' in academic_year else academic_year
        records = await db.attendance.find({"student_id": student_oid, "date": {"$regex": f"^{year_start}"}}).to_list(length=None)
    if not records: records = await db.attendance.find({"student_id": student_oid}).sort("date", -1).limit(200).to_list(length=200)
...
async def list_subjects(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    subjects = [
        # Nursery Subjects
        "Health Habits", "Social Development", "Life Skills Development",
        "Creative Activities", "Language Development", "Numeracy",
        # Primary Subjects
        ...
        # Secondary Core Compulsory
        "English Language", "Mathematics", "Christian Religious Education",
        "Citizenship",
        # Secondary Science Section
        "Biology", "Chemistry", "Physics",
        "Information and Communication Technology (ICT)", "Additional Mathematics",
        "Agriculture Science",
        # Secondary Arts Section
        "History", "Geography", "Commerce", "Accounting", "Literature in English",
        "Kiswahili", "Arabic Language", "French Language", "Fine Art",
    ]
...
async def get_grading_systems(current_user: Dict[str, Any] = Depends(get_current_user)):
    systems = [{"_id": "default", "name": "Standard Grading System", "grade_boundaries": [{"grade": "A", "min_score": 80, "max_score": 100, "remarks": "Excellent", "gpa": 4.0}, {"grade": "B", "min_score": 70, "max_score": 79, "remarks": "Very Good", "gpa": 3.0}, {"grade": "C", "min_score": 60, "max_score": 69, "remarks": "Good", "gpa": 2.0}, {"grade": "D", "min_score": 50, "max_score": 59, "remarks": "Satisfactory", "gpa": 1.0}, {"grade": "F", "min_score": 0, "max_score": 49, "remarks": "Fail", "gpa": 0.0}], "is_default": True}]
...
async def get_exam(exam_id: str = Path(...), current_user: Dict[str, Any] = Depends(get_current_user)):
    db = get_database(); obj_id = _safe_objectid(exam_id)
...
async def create_exam(request: Request, current_user: Dict[str, Any] = Depends(require_role("admin", "teacher"))):
    ...
    exam_name = (body.get("exam_name") or "").strip(); exam_type = (body.get("exam_type") or "").strip()
    class_id = body.get("class_id", ""); subject_id = body.get("subject_id", ""); exam_date = body.get("exam_date", "")
    ...
    sid = _safe_objectid(subject_id); subject_id_value = sid if sid else subject_id
    ...
    max_score = float(body.get("max_score", 100)); pass_mark = body.get("pass_mark", max_score * 0.5)
    uid = _safe_objectid(current_user.get("_id")) if current_user.get("_id") else None
    doc = {"exam_name": exam_name, "exam_type": exam_type, "class_id": cid, "subject_id": subject_id_value, "exam_date": date_obj, "max_score": max_score, "pass_mark": float(pass_mark), "weight": float(body.get("weight", 1.0)), "start_time": body.get("start_time"), "end_time": body.get("end_time"), "academic_year": body.get("academic_year"), "term": body.get("term"), "instructions": body.get("instructions"), "status": "scheduled", "results_entered": 0, "total_students": 0, "created_by": uid, "created_at": datetime.utcnow(), "updated_at": datetime.utcnow()}
    doc = {k: v for k, v in doc.items() if v is not None}
    try:
        result = await db.exams.insert_one(doc); doc["_id"] = str(result.inserted_id)
        return {"success": True, "message": "Exam created", "data": parse_mongo_document(doc)}
    except Exception as e: raise HTTPException(status_code=500, detail=f"Failed to create exam: {str(e)}")
...
async def update_exam(exam_id: str = Path(...), request: Request = None, current_user: Dict[str, Any] = Depends(require_role("admin", "teacher"))):
    ...
    except Exception as e: raise HTTPException(status_code=500, detail=f"Failed to update exam: {str(e)}")
...
async def permanently_delete_exam(exam_id: str = Path(...), current_user: Dict[str, Any] = Depends(require_role("admin"))):
    ...
    try:
        results_deleted = await db.exam_results.delete_many({"exam_id": obj_id}); await db.exams.delete_one({"_id": obj_id})
        await db.audit_log.insert_one({"table_name": "exams", "record_id": exam_id, "operation": "DELETE_PERMANENT", "changed_by": _safe_objectid(current_user.get("_id")) if current_user.get("_id") else None, "details": {"exam_name": exam_name, "exam_type": exam.get("exam_type"), "results_deleted": results_deleted.deleted_count}, "changed_at": datetime.utcnow()})
        return {"success": True, "message": f"Exam '{exam_name}' permanently deleted with {results_deleted.deleted_count} results"}
    except Exception as e: raise HTTPException(status_code=500, detail=f"Failed to delete exam: {str(e)}")
...
async def generate_report_card(request: Request, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    results = await db.exam_results.find({"student_id": student_oid, "term": term, "academic_year": academic_year}).to_list(length=None)
    if not results: results = await db.exam_results.find({"student_id": student_oid}).to_list(length=None)
    ...
    for r in results:
        ...
        if subject_name not in subject_results: subject_results[subject_name] = {"name": subject_name, "score": 0, "max_score": 0, "grade": "N/A", "remarks": ""}
        subject_results[subject_name]["score"] += float(r.get("score", 0)); subject_results[subject_name]["max_score"] += float(exam.get("max_score", 100))
    ...
    for name, data in subject_results.items():
        ...
        data["percentage"] = round(percentage, 1); data["grade"] = _calculate_grade(percentage); data["score"] = round(data["score"], 1)
        subjects_list.append(data); total_score += data["score"]; total_max += data["max_score"]
    overall_percentage = round((total_score / total_max * 100), 1) if total_max > 0 else 0; overall_grade = _calculate_grade(overall_percentage)
    school = await db.school_info.find_one({}) or {}; attendance_data = await _get_student_attendance(db, student_oid, term, academic_year)
    position = body.get("position"); out_of = body.get("out_of")
    if not position and class_id: position, out_of = await _calculate_position(db, student_oid, class_id, overall_percentage, term, academic_year)
    ...
    is_nursery = _is_nursery_class(class_name); is_testimonial = _is_testimonial_class(class_name); grad_type = _get_graduation_type(class_name)
    return {
        ...
        "data": {
            ...
            "results": {"subjects": subjects_list, "total_score": total_score, "total_max": total_max, "percentage": overall_percentage, "grade": overall_grade, "position": position, "out_of": out_of, "result": "Pass" if overall_percentage >= 50 else "Fail", "remarks": remarks, "conduct": body.get("conduct", "Good")},
            "term": term, "academic_year": academic_year, "verify_url": verify_url, "attendance": attendance_data,
            "is_nursery": is_nursery, "is_testimonial": is_testimonial, "graduation_type": grad_type,
            "school": {"name": school.get("school_name", "Heavenly Nature Nursery & Primary School"), "address": school.get("address", ""), "phone": school.get("phone", ""), "email": school.get("email", ""), "motto": school.get("motto", "Nurturing Right Leaders"), "logo_url": school.get("logo_url", "/logo.png")}
        }
    }
...
async def generate_annual_report_card(request: Request, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    async def build_term_results(term_name):
        ...
        for r in term_results:
            exam = exam_map.get(str(r.get("exam_id", "")), {})
            subject_name = exam.get("subject_name") or exam.get("exam_name", "Unknown")
            if subject_name not in subject_results: subject_results[subject_name] = {"name": subject_name, "score": 0, "max_score": 0}
            subject_results[subject_name]["score"] += float(r.get("score", 0)); subject_results[subject_name]["max_score"] += float(exam.get("max_score", 100))
        subjects_list = []; total_score = 0; total_max = 0
        for name, data in subject_results.items():
            percentage = (data["score"] / data["max_score"] * 100) if data["max_score"] > 0 else 0
        ...
        if not term_position and class_id: term_position, term_out_of = await _calculate_position(db, student_oid, class_id, overall_percentage, term_name, academic_year)
        ...
        return {"subjects": subjects_list, "total_score": total_score, "total_max": total_max, "percentage": overall_percentage, "grade": _calculate_grade(overall_percentage), "position": term_position or "N/A", "out_of": term_out_of or "N/A", "result": "Pass" if overall_percentage >= 50 else "Fail", "remarks": term_remarks, "conduct": get_body_value(f"conduct_{term_name.lower().replace(' ', '_')}", "Good")}
    term1_data = await build_term_results("Term 1"); term2_data = await build_term_results("Term 2"); term3_data = await build_term_results("Term 3")
    ...
    if not annual_remarks:
        ...
        if annual_average >= 50: annual_remarks += f" Student will proceed to {next_class} in the next academic year."
        else: annual_remarks += f" Student is advised to repeat {class_name} for a stronger foundation."
    ...
    return {
        ...
        "data": {
            ...
            "annual_summary": {"average_percentage": annual_average, "grade": _calculate_grade(annual_average), "remarks": annual_remarks, "result": "Pass" if annual_average >= 50 else "Fail", "next_class": _get_next_class_name(class_name) if annual_average >= 50 else class_name, "promotion_status": "Promoted" if annual_average >= 50 else "Repeat"},
...
async def get_results(exam_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    exam = await db.exams.find_one({"_id": obj_id})
    if not exam: raise HTTPException(status_code=404, detail="Exam not found")
    existing_results = await db.exam_results.find({"exam_id": obj_id}).to_list(length=None); existing_results = [parse_mongo_document(r) for r in existing_results]
    class_id = exam.get("class_id"); students = []
    if class_id: students = await db.students.find({"current_class_id": class_id, "status": "active"}).to_list(length=None); students = [parse_mongo_document(s) for s in students]
    student_results = []
    for student in students:
        existing = next((r for r in existing_results if str(r.get("student_id")) == str(student.get("_id"))), None)
        student_results.append({"student_id": str(student.get("_id")), "student_name": f"{student.get('first_name', '')} {student.get('last_name', '')}".strip(), "score": existing.get("score") if existing else None, "remarks": existing.get("remarks", "") if existing else "", "grade": existing.get("grade") if existing else None})
    scores = [r["score"] for r in existing_results if r.get("score") is not None]
    stats = {"total_students": len(students), "results_entered": len(existing_results), "highest_score": max(scores) if scores else 0, "lowest_score": min(scores) if scores else 0, "average_score": round(sum(scores) / len(scores), 2) if scores else 0, "pass_rate": round(sum(1 for r in existing_results if r.get("is_passed")) / len(existing_results) * 100, 2) if existing_results else 0}
    return {"success": True, "message": "Results retrieved", "data": {"exam_name": exam.get("exam_name", ""), "class_name": exam.get("class_name", ""), "subject_name": exam.get("subject_name", exam.get("exam_name", "")), "max_score": exam.get("max_score", 100), "pass_mark": exam.get("pass_mark", 50), "students": student_results, "results": existing_results, "statistics": stats}}
...
async def delete_exam_results(exam_id: str = Path(...), current_user: Dict[str, Any] = Depends(require_role("admin"))):
    db = get_database(); obj_id = _safe_objectid(exam_id)
    if not obj_id: raise HTTPException(status_code=400, detail="Invalid exam ID")
    result = await db.exam_results.delete_many({"exam_id": obj_id}); await db.exams.update_one({"_id": obj_id}, {"$set": {"results_entered": 0, "status": "scheduled", "updated_at": datetime.utcnow()}})
...
async def record_results(request: Request, current_user: Dict[str, Any] = Depends(require_role("admin", "teacher"))):
    ...
    if not exam: raise HTTPException(status_code=404, detail="Exam not found")
    max_score = exam.get("max_score", 100); pass_mark = exam.get("pass_mark", 50); term = exam.get("term", ""); academic_year = exam.get("academic_year", "")
    successful = 0
    for r in results:
        try:
            score = float(r.get("score", 0)); percentage = (score / max_score) * 100 if max_score > 0 else 0; grade = _calculate_grade(percentage)
            student_id = _safe_objectid(r.get("student_id"))
            if not student_id: continue
            await db.exam_results.update_one({"exam_id": eid, "student_id": student_id}, {"$set": {"score": score, "grade": grade, "percentage": round(percentage, 2), "is_passed": score >= pass_mark, "remarks": r.get("remarks", ""), "term": term, "academic_year": academic_year, "recorded_by": _safe_objectid(current_user.get("_id")), "updated_at": datetime.utcnow()}}, upsert=True)
            successful += 1
        except Exception as e: print(f"Error recording result: {e}")
    total_results = await db.exam_results.count_documents({"exam_id": eid}); await db.exams.update_one({"_id": eid}, {"$set": {"results_entered": total_results, "status": "completed", "updated_at": datetime.utcnow()}})
