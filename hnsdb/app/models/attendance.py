...
class AttendanceModel:
    ...
    def get_schema() -> Dict[str, Any]:
        ...
        return {
            ...
            "academic_year": "String - Academic year (e.g., 2026/2027)",
    ...
    async def create_indexes(db: AsyncIOMotorDatabase):
        ...
        try:
            await db.attendance.create_index(
                [("student_id", 1), ("class_id", 1), ("date", 1), ("term", 1), ("academic_year", 1)],
                unique=True, name="idx_unique_attendance"
            )
            ...
            await db.attendance.create_index([("class_id", 1), ("date", 1), ("status", 1)], name="idx_attendance_report")
    ...
    async def mark_attendance(
        db: AsyncIOMotorDatabase, student_id: str, class_id: str,
        attendance_date: date, status: str, recorded_by: str,
        notes: Optional[str] = None, arrival_time: Optional[datetime] = None,
        term: Optional[str] = None, academic_year: Optional[str] = None
        ...
        if not term: term = AttendanceModel._get_current_term()
        if not academic_year: academic_year = AttendanceModel._get_current_academic_year()
        
        attendance_record = {
            "student_id": ObjectId(student_id), "class_id": ObjectId(class_id),
            "date": datetime.combine(attendance_date, datetime.min.time()),
            "status": status, "recorded_by": ObjectId(recorded_by),
            "notes": notes, "term": term, "academic_year": academic_year,
            "is_excused_verified": False if status == "excused" else None,
            "created_at": datetime.utcnow(), "updated_at": datetime.utcnow()
        }
        if status == "late" and arrival_time: attendance_record["arrival_time"] = arrival_time
        ...
        try:
            ...
            update_query = {
                "$set": {"status": status, "notes": notes, "recorded_by": ObjectId(recorded_by), "updated_at": datetime.utcnow()}
            }
            if status == "late" and arrival_time: update_query["$set"]["arrival_time"] = arrival_time
            
            result = await db.attendance.update_one(filter_query, update_query, upsert=True)
            await AttendanceModel._log_audit(db, "attendance", str(result.upserted_id) if result.upserted_id else student_id, "UPSERT", recorded_by, {"status": status, "date": str(attendance_date)})
            ...
            if record:
                record["_id"] = str(record["_id"]); record["student_id"] = str(record["student_id"])
                record["class_id"] = str(record["class_id"]); record["recorded_by"] = str(record["recorded_by"])
    ...
    async def bulk_mark_attendance(
        db: AsyncIOMotorDatabase, class_id: str, attendance_date: date,
        attendance_data: List[Dict[str, str]], recorded_by: str,
        term: Optional[str] = None, academic_year: Optional[str] = None
        ...
        for data in attendance_data:
            success, message, _ = await AttendanceModel.mark_attendance(
                db=db, student_id=data.get("student_id"), class_id=class_id,
                attendance_date=attendance_date, status=data.get("status", "present"),
                recorded_by=recorded_by, notes=data.get("notes"),
                term=term, academic_year=academic_year
            )
            if success: successful += 1
            else: failed += 1; errors.append(f"Student {data.get('student_id')}: {message}")
        return successful, failed, errors
    
    @staticmethod
    async def get_student_attendance(
        db: AsyncIOMotorDatabase, student_id: str, start_date: Optional[date] = None,
        end_date: Optional[date] = None, term: Optional[str] = None,
        academic_year: Optional[str] = None, limit: int = 100, skip: int = 0
    ) -> Dict[str, Any]:
        """Get attendance records for a specific student"""
        filter_query = {"student_id": ObjectId(student_id)}
        if start_date: filter_query["date"] = {"$gte": datetime.combine(start_date, datetime.min.time())}
        if end_date:
        ...
        cursor = db.attendance.find(filter_query).sort("date", -1).skip(skip).limit(limit)
        records = []
        ...
        summary = await AttendanceModel._calculate_attendance_summary(db, filter_query)
        return {"records": records, "total": total_count, "limit": limit, "skip": skip, "summary": summary}
    
    @staticmethod
    async def get_class_attendance(
        db: AsyncIOMotorDatabase, class_id: str, attendance_date: Optional[date] = None,
        term: Optional[str] = None, academic_year: Optional[str] = None
    ) -> Dict[str, Any]:
        """Get attendance for an entire class"""
        filter_query = {"class_id": ObjectId(class_id)}
        if attendance_date: filter_query["date"] = datetime.combine(attendance_date, datetime.min.time())
        if term: filter_query["term"] = term
        if academic_year: filter_query["academic_year"] = academic_year
        ...
        attendance_lookup = {str(record["student_id"]): record for record in attendance_records}
        
        student_attendance = []
        present_count = absent_count = excused_count = late_count = unmarked_count = 0
        ...
        for student in students:
            ...
            if record:
                ...
                student_attendance.append({
                    "student_id": student_id, "student_name": f"{student['first_name']} {student['last_name']}",
                    "status": status, "status_display": AttendanceModel.STATUS_DISPLAY.get(status, status),
                    "notes": record.get("notes"), "arrival_time": record.get("arrival_time"), "recorded": True
                })
            else:
                unmarked_count += 1
                student_attendance.append({
                    "student_id": student_id, "student_name": f"{student['first_name']} {student['last_name']}",
                    "status": "unmarked", "status_display": "Not Marked",
                    "notes": None, "arrival_time": None, "recorded": False
                })
        
        return {
            "class_id": class_id, "date": str(attendance_date) if attendance_date else None,
            "students": student_attendance,
            "statistics": {
                "total_students": len(students), "present": present_count,
                "absent": absent_count, "excused": excused_count,
                "late": late_count, "unmarked": unmarked_count,
                "attendance_rate": round(((present_count + late_count) / len(students) * 100) if students else 0, 2)
            }
        }
    ...
    async def get_attendance_report(
        ...
        student_id: Optional[str] = None, start_date: Optional[date] = None,
        end_date: Optional[date] = None, term: Optional[str] = None,
        academic_year: Optional[str] = None
    ) -> Dict[str, Any]:
        """Generate detailed attendance report"""
        match_stage = {}
        if class_id: match_stage["class_id"] = ObjectId(class_id)
        if student_id: match_stage["student_id"] = ObjectId(student_id)
        if start_date: match_stage["date"] = {"$gte": datetime.combine(start_date, datetime.min.time())}
        if end_date:
            if "date" not in match_stage: match_stage["date"] = {}
        ...
        pipeline = [{"$match": match_stage}, {"$group": {"_id": "$status", "count": {"$sum": 1}, "dates": {"$push": "$date"}}}]
        ...
        status_summary = {}; total_records = 0
        for item in status_counts:
            status_summary[item["_id"]] = {"count": item["count"], "display": AttendanceModel.STATUS_DISPLAY.get(item["_id"], item["_id"])}
            total_records += item["count"]
        for status in status_summary:
            status_summary[status]["percentage"] = round((status_summary[status]["count"] / total_records * 100) if total_records else 0, 2)
        
        return {
            "status_summary": status_summary, "total_records": total_records, "daily_trend": daily_trend,
            "attendance_rate": round(((status_summary.get("present", {}).get("count", 0) + status_summary.get("late", {}).get("count", 0)) / total_records * 100) if total_records else 0, 2)
        }
    
    @staticmethod
    async def verify_excuse(
        db: AsyncIOMotorDatabase, attendance_id: str, verified_by: str, is_verified: bool = True
    ) -> Tuple[bool, str]:
        ...
        if attendance["status"] != "excused": return False, "Can only verify excused absences"
        
        await db.attendance.update_one(
            {"_id": ObjectId(attendance_id)},
            {"$set": {"is_excused_verified": is_verified, "verified_by": ObjectId(verified_by), "updated_at": datetime.utcnow()}}
        )
    ...
    async def _calculate_attendance_summary(
        db: AsyncIOMotorDatabase, filter_query: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Calculate attendance summary statistics"""
        pipeline = [{"$match": filter_query}, {"$group": {"_id": "$status", "count": {"$sum": 1}}}]
    ...
    async def _log_audit(
        ...
        operation: str, changed_by: str, details: Dict[str, Any]
    ):
        """Log attendance operations to audit log"""
        try:
            await db.audit_log.insert_one({
                "table_name": table_name, "record_id": record_id,
                "operation": operation, "changed_by": ObjectId(changed_by) if changed_by else None,
