...
function ExamResultForm({ students = [], examInfo = { max_score: 100, pass_mark: 50 }, onSubmit, onCancel, loading = false }) {
  const [results, setResults] = useState(
    students.map(s => ({ 
      ...
      student_name: s.student_name || `${s.first_name || ''} ${s.last_name || ''}`.trim() || 'Unknown',
      score: '', 
      remarks: '' 
    }))
  )
  ...
  const updateScore = (studentId, score) => {
    ...
    setResults(prev => prev.map(r => 
      r.student_id === studentId ? { ...r, score: score === '' ? '' : Number(score) } : r
    ))
  ...
  const updateRemarks = (studentId, remarks) => {
    setResults(prev => prev.map(r => 
      r.student_id === studentId ? { ...r, remarks } : r
    ))
  }
  ...
  const getGrade = (score) => {
    if (score === '' || score === null || score === undefined) return { grade: '-', variant: 'gray' }
    const numScore = Number(score)
    const pct = (numScore / examInfo.max_score) * 100
  ...
  const enteredCount = results.filter(r => r.score !== '' && r.score !== null && r.score !== undefined).length
          ...
          <div className="flex items-center gap-4">
            ...
            {enteredCount > 0 && (
              <>
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-blue-600">{enteredCount > 0 ? Math.round((passCount / enteredCount) * 100) : 0}%</p>
          ...
          <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-700 dark:text-yellow-400">
                {unenteredCount} student{unenteredCount > 1 ? 's' : ''} don't have scores entered.
              </p>
              <p className="text-xs text-yellow-600 dark:text-yellow-500">
                Click "Save All Results" again to submit anyway (unentered students will be excluded).
              </p>
            </div>
          </div>
        )}
      ...
      <Card>
        {results.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">Students must be enrolled in this class to enter results.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {results.map((result) => {
              const grade = getGrade(result.score)
              const isEntered = result.score !== '' && result.score !== null && result.score !== undefined
              return (
                <div
                ...
                >
                  ...
                  <div className="w-16 text-center">
                    ...
                    {isEntered && (
                      <div className="mt-1">
                        {Number(result.score) >= examInfo.pass_mark ? (
