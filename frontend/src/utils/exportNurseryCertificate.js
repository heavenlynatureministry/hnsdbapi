/**
 * Export Certificate/Testimonial to Print/PDF
 * A4 Portrait
 * N3 = Certificate of Nursery Education (photo, QR, award date)
 * P8 = Primary Leaving Testimonial (Index Number, Centre Number)
 * S4 = Secondary Leaving Testimonial (Index Number, Centre Number)
 * All content fits on ONE A4 page
 */

export const exportNurseryCertificate = (reportData, type = 'nursery') => {
  ...
  printWindow.document.write(buildDocument(reportData, letterheadUrl, watermarkUrl, type))
...
const BASE_CSS = `
  ...
  @media print {
    html, body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; margin: 0 !important; padding: 0 !important; background: white !important; width: 100% !important; height: 100% !important; overflow: hidden !important; -webkit-transform: none !important; transform: none !important; }
    .no-print { display: none !important; }
    .page-wrapper, .page, .content { page-break-after: avoid !important; page-break-inside: avoid !important; page-break-before: avoid !important; break-after: avoid !important; break-inside: avoid !important; break-before: avoid !important; }
    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Georgia', 'Times New Roman', serif; font-size: 13px; color: #1a1a1a; background: #e5e7eb; padding: 8px; line-height: 1.5; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .title { text-align: center; font-size: 16px; font-weight: bold; margin: 6px 0 4px; text-transform: uppercase; letter-spacing: 3px; color: #1a3a6b; flex-shrink: 0; }
  ...
  .index-info { text-align: center; margin: 4mm 0; font-size: 11px; line-height: 1.6; flex-shrink: 0; }
  .index-info p { margin: 2px 0; }
...
function buildDocument(reportData, letterheadUrl, watermarkUrl, type) {
  ...
  const isTestimonial = type === 'primary_testimonial' || type === 'secondary_testimonial'
  ...
  // ✅ Different titles and wording for each type
  const titles = {
    nursery: 'THE CERTIFICATE OF NURSERY EDUCATION',
    primary_testimonial: 'PRIMARY LEAVING TESTIMONIAL',
    secondary_testimonial: 'SECONDARY LEAVING TESTIMONIAL',
  }
  
  const awardText = {
    nursery: 'Has been awarded the Certificate of Nursery Education',
    primary_testimonial: 'Has sat for the Certificate of Primary Education',
    secondary_testimonial: 'Has sat for the Certificate of Secondary Education',
  }
  
  const verificationLabel = isTestimonial ? 'Testimonial Verification' : 'Certificate Verification'

  // ✅ Testimonial-specific: Index Number and Centre Number
  const indexNumber = reportData.index_number || student?.index_number || ''
  const centreNumber = reportData.centre_number || student?.centre_number || ''
  
  const testimonialInfoHTML = isTestimonial ? `
    <div class="index-info">
      ${indexNumber ? `<p><strong>Index Number:</strong> ${esc(indexNumber)}</p>` : ''}
      ${centreNumber ? `<p><strong>Centre Number:</strong> ${esc(centreNumber)}</p>` : ''}
      <p>and obtained the following results</p>
    </div>
  ` : ''

  return `<!DOCTYPE html><html>
<head><title>${titles[type] || 'Certificate'} - ${esc(student?.name)}</title><meta charset="utf-8"><style>${BASE_CSS}</style></head>
<body><div class="page-wrapper"><div class="page">
  ...
  <div class="content">
    ...
    <div class="title">${titles[type] || 'CERTIFICATE'}</div>
    ...
    <div class="header-row">
      ...
      <div class="middle-info">
        ...
        ${!isTestimonial ? `<p><strong>Class:</strong> ${esc(student?.class_name || 'N/A')}</p>` : ''}
      </div>
      <div class="qr-box">
    ...
    <div class="certificate-text">
      ...
      <p>${awardText[type] || awardText.nursery}</p>
    </div>
    ...
    <div class="footer">
      <p>${esc(school?.name||'School')} | ${esc(academic_year)} | Computer-generated ${isTestimonial ? 'Testimonial' : 'Certificate'}</p>
      ${verify_url ? `<p class="verify-text">${verificationLabel}: ${esc(verify_url)}</p>` : ''}
    </div>
  </div>
</div></div>
<div class="print-toolbar no-print"><button class="btn btn-print" onclick="window.print()">🖨️ Print ${isTestimonial ? 'Testimonial' : 'Certificate'}</button><button class="btn btn-close" onclick="window.close()">✕ Close</button></div>
</body></html>`
}
