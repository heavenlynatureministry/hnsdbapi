 ...
 * A4 Portrait only - Single Term & Annual Report Cards
...
export const exportReportCard = (reportData) => {
  ...
  printWindow.document.write(buildSingleTerm(student, results, term, academic_year, school, letterheadUrl, watermarkUrl, verify_url))
...
export const exportAnnualReportCard = (reportData) => {
  ...
  printWindow.document.write(buildAnnualPortrait(student, term1, term2, term3, academic_year, school, letterheadUrl, watermarkUrl, verify_url, annual_summary))
  finishAndPrint(printWindow)
}
...
// =========================================================================
// BASE CSS – A4 Portrait only, larger fonts, letterhead + watermark
// =========================================================================
const BASE_CSS = `
...
// SINGLE-TERM REPORT
// =========================================================================
function buildSingleTerm(student, results, term, year, school, letterhead, wm, verify) {
...
<body>
      ...
      <div class="content">
        ...
        <table>
          ...
          <tbody>
            ${subjects.map(s => `
              <tr>
                <td><strong>${esc(s.name || s.subject)}</strong></td>
            ...
            <tr class="total-row">
              ...
              <td class="${cls}"><strong>${pass.toUpperCase()}</strong></td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="summary-section">
          ...
          <div class="remarks-box">
            ...
            <p>${esc(results?.remarks) || '________________________________'}</p>
          </div>
        </div>
  ...
  <div class="print-toolbar no-print">
    <button class="btn btn-print" onclick="window.print()">🖨️ Print (A4 Portrait)</button>
...
function buildAnnualPortrait(student, t1, t2, t3, year, school, letterhead, wm, verify, annual) {
  ...
  const rows = subjects.map(subj => {
    ...
    return `<tr>
      <td class="subject-col">${esc(subj)}</td>
      <td>${a.score || '-'}</td>
      <td>${b.score || '-'}</td>
      <td>${c.score || '-'}</td>
    </tr>`
    ...
    <div class="page">
      ...
      <div class="content">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr class="total-row">
              ...
              <td><strong>${t1?.total_score || '-'}</strong></td>
              <td><strong>${t2?.total_score || '-'}</strong></td>
              <td><strong>${t3?.total_score || '-'}</strong></td>
            </tr>
        ...
        <div class="annual-summary">
          ...
          <p><strong>Average:</strong> ${annual.average_percentage || 'N/A'}% | <strong>Grade:</strong> ${annual.grade || 'N/A'}</p>
          <p><strong>Status:</strong> <span class="${annual.promotion_status === 'Promoted' ? 'promoted' : 'repeat'}">${annual.promotion_status || 'N/A'}</span> | <strong>Next Class:</strong> ${esc(annual.next_class || 'N/A')}</p>
          <p><strong>Remarks:</strong> ${esc(annual.remarks || '')}</p>
        </div>
        ` : ''}
        ...
        <div class="next-term"><strong>Next Academic Year:</strong> January ${parseInt(year?.split('/')[1] || new Date().getFullYear() + 1)}</div>
        <div class="footer">
          <p>${esc(school?.name || 'School')} | Annual Report ${esc(year)} | Computer-generated</p>
          ${verify ? `<p style="color:#1a56db;">Verify: ${esc(verify)}</p>` : ''}
        </div>
      </div>
    </div>
