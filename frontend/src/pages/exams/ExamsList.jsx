...
import { 
  FileText, Plus, MoreVertical, Eye, Edit, BarChart3, 
...
function ExamsList() {
  ...
  const fetchExams = async () => {
    ...
    try {
      ...
      const examList = data?.exams || data || []
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/exams/${deleteExam._id}/permanent`)
      toast.success(`Exam "${deleteExam.exam_name}" deleted permanently`)
  ...
  const filteredExams = typeFilter ? safeExams.filter(e => e?.exam_type === typeFilter) : safeExams

  return (
    <div className="space-y-6 animate-fade-in-up">
      <PageHeader
        title="Examinations"
        subtitle={`${safeExams.length} exams`}
        actions={
      ...
      <div className="flex gap-2 flex-wrap">
        {['', 'mid_term', 'end_term', 'quiz', 'mock'].map((type) => (
          <button key={type} onClick={() => setTypeFilter(type)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${typeFilter === type ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}>
            {type ? type.replace('_', ' ') : 'All'} {type && `(${safeExams.filter(e => e?.exam_type === type).length})`}
          </button>
        ))}
      </div>
      ...
      {loading ? <LoadingSpinner /> : filteredExams.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredExams.map((exam) => (
            <Card key={exam?._id || Math.random()} className="hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-semibold text-sm">{exam?.exam_name || 'N/A'}</h3>
                    <p className="text-xs text-gray-500">{exam?.class_name || 'N/A'} • {exam?.subject_name || 'N/A'}</p>
                  </div>
                </div>
              ...
              <div className="space-y-2 text-sm mb-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">{exam?.max_score || 'N/A'}</span>
                </div>
                <div className="flex items-center justify-between">
              ...
              {exam?.status !== 'scheduled' && exam?.total_students > 0 && (
                <div className="mt-2">
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                    <div className="bg-primary-600 h-1.5 rounded-full" style={{ width: `${((exam?.results_entered || 0) / (exam?.total_students || 1)) * 100}%` }} />
                  </div>
                </div>
              )}
            </Card>
      ...
      <ConfirmDialog
        ...
        title="Delete Exam"
        message={`Are you sure you want to permanently delete "${deleteExam?.exam_name}"? This will also delete all results for this exam. This action cannot be undone.`}
