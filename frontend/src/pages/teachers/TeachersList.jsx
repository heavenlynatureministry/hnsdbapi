...
function TeachersList() {
  ...
  const handlePermanentDelete = async () => {
    ...
    try {
      await api.delete(`/teachers/${deleteTeacher._id}/permanent`)
      toast.success(`Teacher ${deleteTeacher.first_name} ${deleteTeacher.last_name} deleted permanently`)
  ...
  const filteredTeachers = specializationFilter
    ? safeTeachers.filter(t => t?.specialization === specializationFilter)
    : safeTeachers
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      {filteredTeachers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredTeachers.map((teacher) => (
            <div key={teacher?._id || Math.random()} className="card hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{teacher?.employee_id || 'N/A'}</p>
                  </div>
                </div>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to permanently delete ${deleteTeacher?.first_name} ${deleteTeacher?.last_name}? This will remove all related records including class assignments, reviews, and leave history. This action cannot be undone.`}
