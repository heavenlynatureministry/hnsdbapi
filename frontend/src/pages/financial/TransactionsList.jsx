...
function TransactionsList() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await financialAPI.deleteTransaction(showDelete._id)
      if (response?.success) {
  ...
  const totalIncome = transactions.filter(t => t.transaction_type === 'income' && t.approval_status === 'approved').reduce((s, t) => s + (t.amount || 0), 0)
  const totalExpenses = transactions.filter(t => t.transaction_type === 'expense' && t.approval_status === 'approved').reduce((s, t) => s + (t.amount || 0), 0)
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
              ...
              <tbody>
                {transactions.map((txn) => (
                  <tr key={txn._id}>
                    ...
                    <td><Badge variant={txn.transaction_type === 'income' ? 'success' : 'danger'}>{txn.transaction_type}</Badge></td>
      ...
      <ConfirmDialog open={!!showDelete} onClose={() => setShowDelete(null)} onConfirm={handleDelete} title="Delete Transaction" message="Are you sure you want to delete this transaction?" confirmText="Delete" variant="danger" />
    </div>
  )
}
